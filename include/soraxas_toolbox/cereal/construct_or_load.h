...
std::shared_ptr<T> construct_or_load(const std::string &cache_fname,
                                     Args &&...constructor_args) {
  static_assert(
      sizeof...(constructor_args) >= 1,
      "Constructing object with zero argunment is explicitly disallowed, as "
      "it is used by cereal for deserialisation.");
  ...
  if (std::filesystem::exists(cache_fname)) {
    ...
    oarchive(target_obj);                     // Write the data to the archive
                          // archive goes out of scope, ensuring all contents
                          // are flushed
  }
  return target_obj;
}
