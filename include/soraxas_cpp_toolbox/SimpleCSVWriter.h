...
public:
  ...
  CSVWriterStream &add(const char *str) { return add(std::string(str)); }
  ...
  template <typename T> CSVWriterStream &add(T str) {
    if (columnNum_ > -1) {
  ...
  template <typename T> CSVWriterStream &operator<<(const T &t) {
    return add(t);
  }

  void operator+=(CSVWriterStream &csv) { ss_ << std::endl << csv; }
  ...
  CSVWriterStream &newRow() {
    ...
    if (writeImmediately_) {
      // write to file without auto-adding newline (as it will be handled on a
      // per-line basis)
  ...
  bool writeToFile(std::string filename, bool append = false,
  ...
  void disableWriteImmediately() { writeImmediately_ = false; }

  void enableAutoNewRow(int numberOfColumns) { columnNum_ = numberOfColumns; }
...
public:
  CSVInstantWriter(std::string filename, bool append = false,
                   std::string seperator = ",")
      : CSVWriterStream(","), writeImmediatelyFilename_(filename) {
    if (!append) {
      // clear file content (by opening it)
      std::ofstream file;
      file.open(writeImmediatelyFilename_.c_str(),
                std::ios::out | std::ios::trunc);
      file.close();
    }
  }

  template <typename T, typename... Args> void addNewRow(T t, Args... args) {
    add(t);
    addNewRow(args...);
  }
